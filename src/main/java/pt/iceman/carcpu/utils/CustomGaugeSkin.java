...
public class CustomGaugeSkin extends SkinBase<Gauge>implements Skin<Gauge>
{
	...
	private void init()
	{
		if (Double.compare(getSkinnable().getPrefWidth(), 0.0) <= 0 || Double.compare(getSkinnable().getPrefHeight(), 0.0) <= 0 || Double.compare(getSkinnable().getWidth(), 0.0) <= 0 || Double.compare(getSkinnable().getHeight(), 0.0) <= 0)
		{
	...
	private void initGraphics()
	{
		...
		// Add all nodes
		pane = new Pane();
		pane.getChildren().setAll(background, histogram, ticksAndSectionsCanvas, minMeasuredValue, maxMeasuredValue, threshold,
	...
	private void rotateNeedle()
	{
		...
		{
			// double animationDuration = (getSkinnable().getAnimationDuration()
			// / (getSkinnable().getMaxValue() - getSkinnable().getMinValue()))
			// * Math.abs(getSkinnable().getValue() -
			// getSkinnable().getOldValue());
	...
	private void resize()
	{
		...
		if (size > 0)
		{
			...
			// TODO: Remove this CSS inlining workaround by using % in
			// -fx-background-insets
			// background.setStyle("-fx-background-insets: 0, " + 0.018 * size +
			// ", " + 0.02 * size + ", " + 0.036 * size + ";");

			ticksAndSectionsCanvas.setWidth(size);
			ticksAndSectionsCanvas.setHeight(size);
			ticksAndSections.clearRect(0, 0, size, size);
			if (getSkinnable().isSectionsVisible())
				drawSections(ticksAndSections);
			if (getSkinnable().isAreasVisible())
				drawAreas(ticksAndSections);
			drawTickMarks(ticksAndSections);
			ticksAndSectionsCanvas.setCache(true);
			ticksAndSectionsCanvas.setCacheHint(CacheHint.QUALITY);

			if (getSkinnable().isMarkersVisible())
				drawMarkers();
